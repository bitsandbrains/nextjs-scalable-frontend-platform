...
import { getTaskStatus, TaskStatus as TaskStatusType } from '@/lib/api';
...
export default function JobStatus({ taskId, onComplete }: JobStatusProps) {
    ...
    return (
        <div className="text-center py-6 fade-in">
            <Loader2 className="spinner text-orange-600 mx-auto mb-4" size={48} strokeWidth={2} />
            ...
            <div className="w-full bg-gray-200 rounded-full h-3 max-w-md mx-auto overflow-hidden shadow-inner">
                <div
                    className="progress-bar h-3 rounded-full transition-all duration-500 ease-out"
                    style={{ width: `${progress}%` }}
                />
            </div>
            <p className="text-sm font-semibold text-orange-600 mt-2">{Math.round(progress)}% complete</p>
        </div>
    );
}
